...
- **渲染器按内容本性选择**,选定后全页一致、不混用;默认不手写原生 WebGL/Canvas 绘图:
...
- 用户说"存进 wiki / 沉淀进知识库"时:用 `vite-plugin-singlefile` 导出自包含单文件版本,归档交给 `wiki-html` 技能。
...
简报:文件路径、课程 slug、核心困惑是什么、页面怎么攻破它(主 demo 演示什么、拆了几步、证伪了哪个误解)、留了哪些可调参数和挑战、验证结果;提醒双击即可打开。
